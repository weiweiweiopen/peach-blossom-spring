...
export function renderAssociationFeedbackSection(language: AssociationFeedbackLanguage, templateFilename = "01-pbs-reset-title-kinetic.html"): string {
  const copy: Record<AssociationFeedbackLanguage, { title: string; feedback: string; submit: string; submitted: string; pdf: string; pdfLabel: string }> = {
    "zh-TW": { title: "向 LLM 提交故障排修資訊和人工檢驗輔助", feedback: "請直接寫下一版要怎麼修：有用段落、無用或誤導處、缺少證據、希望保留或刪除的方向。", submit: "提交排修並重生小誌", submitted: "已送出，正在重生小誌...", pdf: "列印 / 存 PDF", pdfLabel: "列印或存成 PDF" },
    en: { title: "Send repair notes and human review aid to the LLM", feedback: "Write how the next version should be repaired: useful parts, misleading or weak parts, missing evidence, and what to keep or remove.", submit: "Submit repair and regenerate", submitted: "Submitted. Regenerating zine...", pdf: "Print / Save PDF", pdfLabel: "Print or save PDF" },
    id: { title: "Kirim catatan perbaikan dan bantuan tinjauan manusia ke LLM", feedback: "Tulis bagaimana versi berikutnya perlu diperbaiki: bagian berguna, bagian lemah, bukti yang hilang, dan apa yang perlu disimpan atau dihapus.", submit: "Kirim perbaikan dan buat ulang", submitted: "Terkirim. Membuat ulang zine...", pdf: "Cetak / PDF", pdfLabel: "Cetak atau simpan PDF" },
    de: { title: "Reparaturhinweise und menschliche Pruefung an das LLM senden", feedback: "Schreibe, wie die naechste Version repariert werden soll: Nuetzliches, Schwaches, fehlende Belege und was bleiben oder wegfallen soll.", submit: "Reparatur senden und neu erzeugen", submitted: "Gesendet. Zine wird neu erzeugt...", pdf: "Drucken / PDF", pdfLabel: "Drucken oder als PDF speichern" },
    ja: { title: "修復情報と人間による検証補助を LLM に送る", feedback: "次の版をどう直すかを書いてください。役立つ部分、弱い部分、不足する証拠、残す／消す方向を一つの欄にまとめます。", submit: "修復を送信して再生成", submitted: "送信しました。小誌を再生成しています...", pdf: "印刷 / PDF", pdfLabel: "印刷またはPDF保存" },
    th: { title: "ส่งข้อมูลซ่อมและตัวช่วยตรวจมนุษย์ให้ LLM", feedback: "เขียนว่าฉบับถัดไปควรซ่อมอย่างไร: ส่วนที่มีประโยชน์ ส่วนที่อ่อน หลักฐานที่ขาด และสิ่งที่ควรเก็บหรือตัดออก", submit: "ส่งการซ่อมและสร้างใหม่", submitted: "ส่งแล้ว กำลังสร้างซีนใหม่...", pdf: "พิมพ์ / PDF", pdfLabel: "พิมพ์หรือบันทึก PDF" },
  };
  const selected = copy[language];
  const fieldStyle = "width:100%;min-height:180px;resize:vertical;border:3px solid #000;border-radius:0;padding:14px 16px;background:#fff;color:#243b3d;box-shadow:4px 4px 0 #000;font:inherit;font-size:clamp(15px,1.8vw,20px);line-height:1.45;outline:0;box-sizing:border-box;";
  return `<section class="page zine-feedback-page" data-folio="feedback" style="break-before:auto;page-break-before:auto;min-height:auto;display:block;padding:clamp(14px,3vw,28px);background:#fffaf0;color:#243b3d;"><div class="zine-system-frame">
    <form data-pbs-zine-repair-form style="display:grid;grid-template-columns:minmax(0,1fr);gap:14px;align-items:stretch;margin:0;">
      ...
      <textarea data-pbs-zine-repair-feedback aria-label="${escapeHtml(selected.feedback)}" placeholder="${escapeHtml(selected.feedback)}" rows="6" style="${fieldStyle}"></textarea>
...
(() => {
  ...
  document.querySelectorAll("[data-pbs-zine-repair-form]").forEach((form) => {
    form.addEventListener("submit", (event) => {
      ...
      const feedback = form.querySelector("[data-pbs-zine-repair-feedback]")?.value || "";
      const entry = {
        usefulParts: "",
        uselessParts: "",
        repairInstruction: feedback,
