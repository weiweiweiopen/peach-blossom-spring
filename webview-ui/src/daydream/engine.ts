...
export function generateDaydreamReport(seed: string, corpus: DaydreamCorpus): DaydreamReport {
  ...
  const futures = buildFutures(seed, keywords, evidenceCards, depthMetrics);
...
function expandViaGraph(seedCards: SourceCard[], corpus: DaydreamCorpus): SourceCard[] {
  ...
  for (const edge of corpus.edges) {
    if (!seedIds.has(edge.source)) continue;
    const boost = edge.relation === "tagged" ? 3 : 2;
    graphTargets.set(edge.target, (graphTargets.get(edge.target) ?? 0) + boost * (edge.weight ?? 1));
  }

  const related = new Map<string, number>();
  for (const edge of corpus.edges) {
    const targetWeight = graphTargets.get(edge.target);
    if (!targetWeight || seedIds.has(edge.source)) continue;
...
function resolveOutgoingEdgeTargets(card: SourceCard, corpus: DaydreamCorpus): SourceCard[] {
