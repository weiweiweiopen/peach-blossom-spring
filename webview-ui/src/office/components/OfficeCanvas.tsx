...
export function OfficeCanvas({
    ...
    const stop = startGameLoop(canvas, {
      ...
      render: (ctx) => {
        ...
        const { offsetX, offsetY } = renderFrame(
          ...
          officeState.getLayout().pixelMapBackground,
        );
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      unlockAudio();
      // Middle mouse, or left-drag on empty map, pans the world.
      if (e.button === 1 || (!isEditMode && e.button === 0)) {
  ...
  const handleMouseUp = useCallback(
    (e: React.MouseEvent) => {
      if (isPanningRef.current && (e.button === 0 || e.button === 1)) {
        ...
        suppressNextClickRef.current = panMovedRef.current;
        panMovedRef.current = false;
        const canvas = canvasRef.current;
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (touchPointersRef.current.size >= 2) {
        ...
        officeState.cameraFollowId = null;
        touchPanRef.current = { x: center.x, y: center.y, panX: panRef.current.x, panY: panRef.current.y };
    ...
    [isEditMode, mobileTapToMove, officeState, panRef, screenToTile, stopMobileHoldMove],
  );
  ...
  const handlePointerUp = useCallback(
    (e: React.PointerEvent) => {
      if (e.pointerType === 'touch') {
        ...
        if (touchPointersRef.current.size < 2) touchPanRef.current = null;
      }
