...
const PLAYER_SPRINT_WALK_HOLD_SEC = 0.14;
...
export class OfficeState {
  ...
  addQuestionPet(id: number, name: string, spriteOverride: SpriteData, col = 32, row = 32, spriteAnimation?: SpriteData[], imageSpriteAnimation?: string[]): void {
    if (this.characters.has(id)) return;
    ...
    ch.spriteAnimationsByDirection = undefined;
    ch.imageSpriteAnimation = imageSpriteAnimation;
    ch.imageSpriteSize = undefined;
    ch.isActive = false;
    ch.state = CharacterState.IDLE;
    ch.seatId = null;
    ch.matrixEffect = null;
  ...
  movePlayerBy(id: number, dCol: number, dRow: number): boolean {
    ...
    // Keep a tiny queue so sprinting stays in WALK instead of flickering back to IDLE between tiles.
  ...
  private updatePlayer(ch: Character, dt: number): void {
    const walkFrameCount = ch.spriteAnimationsByDirection?.[ch.dir]?.length
      ?? ch.imageSpriteAnimation?.length
      ?? ch.spriteAnimation?.length
      ?? 4;

    if (ch.path.length === 0) {
      const holdTimer = ch.walkAnimationHoldTimer ?? 0;
      if (holdTimer > 0) {
        ch.state = CharacterState.WALK;
        ch.walkAnimationHoldTimer = Math.max(0, holdTimer - dt);
        ch.frameTimer += dt;
        if (ch.frameTimer >= WALK_FRAME_DURATION_SEC) {
          ch.frameTimer -= WALK_FRAME_DURATION_SEC;
          ch.frame = (ch.frame + 1) % walkFrameCount;
        }
        return;
      }
      ch.state = CharacterState.IDLE;
    ...
    if (dist <= step || dist === 0) {
      ...
      if (speedMultiplier > 1) {
        ch.frame = (ch.frame + 1) % walkFrameCount;
        ch.walkAnimationHoldTimer = PLAYER_SPRINT_WALK_HOLD_SEC;
      }
      if (ch.path.length === 0) {
        ch.state = speedMultiplier > 1 ? CharacterState.WALK : CharacterState.IDLE;
        ch.frame = speedMultiplier > 1 ? ch.frame : 0;
        ch.frameTimer = 0;
      }
      return;
    }
