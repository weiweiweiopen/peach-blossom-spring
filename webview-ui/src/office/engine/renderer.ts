...
const imageSpriteCache = new Map<string, HTMLImageElement>();
const pixelMapCache = new WeakMap<PixelMapBackground, HTMLCanvasElement>();

function getPixelMapCanvas(background: PixelMapBackground): HTMLCanvasElement | null {
  if (typeof document === 'undefined') return null;
  const cached = pixelMapCache.get(background);
  ...
  const imageData = ctx.createImageData(background.width, background.height);
  for (let y = 0; y < background.height; y++) {
    const row = background.rows[y] ?? '';
    for (let x = 0; x < background.width; x++) {
      const color = background.palette[row[x] ?? '.'] ?? background.palette['.'] ?? '#000000';
      const out = (y * background.width + x) * 4;
      imageData.data[out] = Number.parseInt(color.slice(1, 3), 16);
      imageData.data[out + 1] = Number.parseInt(color.slice(3, 5), 16);
      imageData.data[out + 2] = Number.parseInt(color.slice(5, 7), 16);
      imageData.data[out + 3] = 255;
    }
  }
  ctx.putImageData(imageData, 0, 0);
  pixelMapCache.set(background, canvas);
  return canvas;
}

function renderPixelMapBackground(
  ctx: CanvasRenderingContext2D,
  ...
  const canvas = getPixelMapCanvas(background);
  if (!canvas) return;
  ctx.save();
  ctx.imageSmoothingEnabled = false;
  ctx.drawImage(canvas, offsetX, offsetY, mapW, mapH);
  ctx.restore();

  if (!background.labels?.length) return;
  const scaleX = mapW / background.width;
  const scaleY = mapH / background.height;
  ctx.save();
  ctx.imageSmoothingEnabled = false;
  ctx.font = `${Math.max(8, Math.round(scaleY * 4))}px monospace`;
  ctx.textBaseline = 'top';
  ctx.lineWidth = 3;
  ctx.strokeStyle = '#F9E9C2';
  for (const label of background.labels) {
    const x = offsetX + label.x * scaleX;
    const y = offsetY + label.y * scaleY;
    ctx.fillStyle = label.color ?? '#141414';
...
function getImageSprite(src: string): HTMLImageElement | null {
  if (typeof Image === 'undefined') return null;
  const cached = imageSpriteCache.get(src);
  if (cached) return cached;
  const image = new Image();
  image.src = src;
  imageSpriteCache.set(src, image);
  return image;
}
...
export function renderScene(
  ...
  for (const ch of characters) {
    const imageFrame = ch.imageSpriteAnimation?.[ch.frame % ch.imageSpriteAnimation.length];
    if (imageFrame) {
      const image = getImageSprite(imageFrame);
      if (image?.complete && image.naturalWidth > 0 && image.naturalHeight > 0) {
        const sittingOffset = ch.state === CharacterState.TYPE ? CHARACTER_SITTING_OFFSET_PX : 0;
        const drawW = ch.imageSpriteSize ?? image.naturalWidth;
        const drawH = ch.imageSpriteSize
          ? image.naturalHeight * (ch.imageSpriteSize / image.naturalWidth)
          : image.naturalHeight;
    ...
    const directionalAnimation = ch.spriteAnimationsByDirection?.[ch.dir];
    const spriteData = directionalAnimation?.[ch.frame % directionalAnimation.length] ?? ch.spriteAnimation?.[ch.frame % ch.spriteAnimation.length] ?? ch.spriteOverride ?? getCharacterSprite(ch, sprites);
...
export function renderFrame(
  ...
  layoutCols?: number,
  layoutRows?: number,
  pixelMapBackground?: PixelMapBackground,
  ...
  if (pixelMapBackground) {
    renderPixelMapBackground(ctx, pixelMapBackground, offsetX, offsetY, mapW, mapH);
  } else {
