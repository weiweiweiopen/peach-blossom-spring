...
export function updateCharacter(
  ...
  switch (ch.state) {
    ...
    case CharacterState.IDLE: {
      const petFrameCount = ch.spriteAnimationsByDirection?.[ch.dir]?.length ?? ch.imageSpriteAnimation?.length ?? ch.spriteAnimation?.length ?? 0;
      if (ch.isQuestionPet && petFrameCount > 0) {
        if (ch.frameTimer >= TYPE_FRAME_DURATION_SEC / 2) {
          ch.frameTimer -= TYPE_FRAME_DURATION_SEC / 2;
          ch.frame = (ch.frame + 1) % petFrameCount;
        }
      } else {
    ...
    case CharacterState.WALK: {
      ...
      if (ch.frameTimer >= WALK_FRAME_DURATION_SEC) {
        ...
        ch.frame = (ch.frame + 1) % (ch.spriteAnimationsByDirection?.[ch.dir]?.length ?? 4);
      }
