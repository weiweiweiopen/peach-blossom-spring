...
function drawRect(sprite: SpriteData, x: number, y: number, w: number, h: number, color: string): void {
  for (let row = Math.max(0, y); row < Math.min(sprite.length, y + h); row++) {
    for (let col = Math.max(0, x); col < Math.min(sprite[row].length, x + w); col++) {
...
function drawTrapezoid(
  ...
  for (let y = y0; y <= y1; y++) {
    ...
    drawRect(sprite, left, y, right - left + 1, 1, color);
  }
}

function drawLine(sprite: SpriteData, x0: number, y0: number, x1: number, y1: number, color: string): void {
  const steps = Math.max(Math.abs(x1 - x0), Math.abs(y1 - y0));
  for (let step = 0; step <= steps; step++) {
    const t = steps === 0 ? 0 : step / steps;
    drawRect(sprite, Math.round(x0 + (x1 - x0) * t), Math.round(y0 + (y1 - y0) * t), 2, 2, color);
  }
}

function createThaiTempleSprite(): SpriteData {
  const sprite = emptySprite(96, 96);
  const black = '#141414';
  const gold = '#FCF46B';
  const goldDark = '#D8B928';
  const cream = '#F9E9C2';
  const pink = '#FFD4FF';
  const blue = '#BAC3D9';
  const teal = '#69C3AA';

  drawRect(sprite, 10, 83, 76, 8, black);
  drawRect(sprite, 14, 78, 68, 10, blue);
  drawRect(sprite, 20, 73, 56, 8, cream);

  drawRect(sprite, 17, 47, 62, 32, black);
  drawRect(sprite, 22, 51, 49, 27, cream);
  drawRect(sprite, 71, 54, 5, 24, '#D3D9E8');
  drawRect(sprite, 24, 52, 45, 4, pink);
  drawRect(sprite, 25, 74, 44, 4, blue);

  drawRect(sprite, 35, 58, 26, 20, black);
  drawRect(sprite, 40, 62, 16, 16, teal);
  drawRect(sprite, 47, 62, 2, 16, black);
  drawRect(sprite, 43, 68, 3, 4, cream);
  drawRect(sprite, 51, 68, 3, 4, cream);

  for (const x of [25, 31, 64, 70]) {
    drawRect(sprite, x, 56, 4, 20, black);
    drawRect(sprite, x + 1, 58, 2, 16, gold);
  }

  drawTrapezoid(sprite, 35, 48, 7, 87, 16, 80, black);
  drawTrapezoid(sprite, 36, 45, 11, 83, 20, 76, pink);
  drawTrapezoid(sprite, 38, 43, 19, 75, 25, 70, gold);
  drawRect(sprite, 6, 42, 10, 4, black);
  drawRect(sprite, 80, 42, 10, 4, black);
  drawRect(sprite, 8, 38, 7, 4, gold);
  drawRect(sprite, 81, 38, 7, 4, gold);

  drawTrapezoid(sprite, 20, 34, 28, 68, 17, 79, black);
  drawTrapezoid(sprite, 22, 31, 31, 65, 24, 72, pink);
  drawTrapezoid(sprite, 24, 29, 36, 60, 31, 65, gold);
  drawRect(sprite, 45, 9, 6, 13, black);
  drawRect(sprite, 47, 6, 2, 18, gold);

  drawLine(sprite, 18, 47, 48, 34, goldDark);
  drawLine(sprite, 78, 47, 48, 34, goldDark);
  drawLine(sprite, 24, 34, 48, 22, goldDark);
  drawLine(sprite, 72, 34, 48, 22, goldDark);
  for (const x of [29, 37, 59, 67]) drawRect(sprite, x, 61, 8, 10, blue);
  for (const y of [82, 88]) drawRect(sprite, 26, y, 44, 3, cream);

  return sprite;
}

function createThaiTempleCatalogEntry(): CatalogEntryWithCategory {
  return {
    type: 'THAI_TEMPLE',
    label: 'Thai Temple',
    footprintW: 6,
    footprintH: 6,
    sprite: createThaiTempleSprite(),
    isDesk: false,
    category: 'misc',
  };
}
...
export function buildDynamicCatalog(assets: LoadedAssetData): boolean {
  ...
  allEntries.push(createThaiTempleCatalogEntry());
