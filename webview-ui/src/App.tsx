...
function App() {
  ...
  useEffect(() => {
    document.title = t(selectedLanguage, "projectTitle");
  }, [selectedLanguage]);
    ...
    simSnapshot.thronglets.forEach((pet, index) => {
      if (!officeState.characters.has(pet.characterId)) {
        officeState.addQuestionPet(
          pet.characterId,
          languageLabel(selectedLanguage, "思考生命體", "Thinking Lifeform"),
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      const nearTree =
        !!player &&
        !!archiveTreeZone &&
        isInZone(player.tileCol, player.tileRow, archiveTreeZone, 1);
  ...
  return (
    <div
      ...
      style={{
        touchAction:
          showMobileControls && appMode === "interactive" ? "none" : undefined,
      }}
      ...
      {!isDebugMode ? (
        <>
              ...
              <div className="absolute inset-x-0 bottom-0 h-[34vh] z-44 border-t-2 border-black bg-white text-black backdrop-blur-[1px] flex items-center justify-center">
                <div className="text-center px-10 max-w-[960px]">
                  <p className="text-lg text-black mb-3">
                    {t(selectedLanguage, "abaoEncounterTitle")}
                  </p>
                  ...
                  <p className="text-base text-black">
                    {t(selectedLanguage, "abaoEncounterHint")}
                  </p>
                </div>
              </div>
            )}
          ...
          {appMode === "interactive" &&
            ...
            !activeDialoguePersona && (
              <section className="absolute right-12 top-12 z-43 w-[min(920px,calc(100vw-24px))] max-h-[calc(100vh-96px)] overflow-auto pixel-panel px-12 py-10 text-text shadow-pixel">
                ...
                <h1 className="text-2xl leading-none mb-3">
                  {t(selectedLanguage, "archiveTitle")}
                </h1>
                <p className="text-sm text-text-muted mb-8">
                  {t(selectedLanguage, "archiveDescription")}
                </p>
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                  <div className="border border-border bg-bg/70 px-7 py-5">
                    <h2 className="text-lg text-accent-bright mb-4">
                      {t(selectedLanguage, "archivePdfTitle")}
                    </h2>
                    <iframe
                  ...
                  <div className="border border-border bg-bg/70 px-7 py-5">
                    <h2 className="text-lg text-accent-bright mb-4">
                      {t(selectedLanguage, "archiveMapTitle")}
                    </h2>
                    <iframe
          ...
          {(selectedDispatchPet || selectedNpcInfo) && (
            <section
              className="question-response-panel info-card absolute right-12 bottom-12 z-51 w-[min(520px,calc(100vw-24px))] px-8 py-7"
              data-no-mobile-drag="true"
            >
              <button
          ...
          {shouldShowMobileStatsBar && (
            <button
              className={`mobile-stats-bar ${isTextInputFocused ? "is-input-focused" : ""}`}
              type="button"
              aria-label={
                mobileRulesOpen
                  ? t(selectedLanguage, "closeDispatchMonitor")
                  : t(selectedLanguage, "openDispatchMonitor")
              }
              onClick={() => setMobileRulesOpen(true)}
              data-no-mobile-drag="true"
            >
              <span>
                {t(selectedLanguage, "mobileDispatchMonitorCollapsed")}
              </span>
          ...
          {mobileRulesOpen && (
            <section
              className={`mobile-rules-drawer ${isTextInputFocused ? "is-input-focused" : ""}`}
              data-no-mobile-drag="true"
            >
              <button
                className="float-right text-xl"
          ...
          {selectedPet && (
            <section
              className="question-response-panel absolute right-12 bottom-12 z-51 w-[min(520px,calc(100vw-24px))] px-8 py-7"
              data-no-mobile-drag="true"
            >
              <button
                className="float-right text-xl"
              ...
              <textarea
                ...
                placeholder={
                  selectedLanguage === "zh-TW"
                    ? "回應這隻思考生命體..."
                    : "Respond to this thinking lifeform..."
                }
              />
              <button
