...
const SCHEMA_CONTROL_COPY: Record<LanguageCode, {
  ...
  "zh-TW": {
    ...
    intro: [
      ...
      "Non-Governmental Matters 已把 Hackteria、跨國科技藝術營隊、獨立教育、資金模式、材料實驗與文化差異整理成第一層田野材料。桃花源把這些材料轉成 AI 時代的知識保存問題：文化組織需要能保存、分類、召回、比較、修正與再使用知識的認知系統。",
  ...
  id: {
    ...
    intro: ["Organisasi budaya kecil dan jaringan seni independen sering bergantung pada orang kunci, hibah singkat, lokakarya sementara, komunikasi informal, dan memori pribadi. Pengetahuan tersebar di wawancara, wiki, folder cloud, dokumen hibah, arsip pameran, bahan lokakarya, media sosial, dan cerita lisan.", "Non-Governmental Matters telah menjadi bahan lapangan awal tentang Hackteria, camp seni-teknologi lintas negara, pendidikan independen, eksperimen material, model pendanaan, dan perbedaan budaya. Peach Blossom Spring mengubahnya menjadi persoalan pelestarian pengetahuan pada era AI.", "Alur sekarang: jelajahi Peach Blossom Spring, bicara dengan memori wawancara NPC, ajukan pertanyaan ke campfire LLM Wiki, lalu buat zine dari public source packet dan promoted wiki memory. Question lint berasal dari traversal shared memory: seberapa spesifik pertanyaan, seberapa siap buktinya, dan seberapa besar potensi lintas sistemnya."],
  ...
  th: {
    ...
    intro: ["องค์กรวัฒนธรรมขนาดเล็กและเครือข่ายศิลปะอิสระมักพึ่งคนสำคัญ ทุนระยะสั้น เวิร์กช็อปชั่วคราว การสื่อสารไม่เป็นทางการ และความทรงจำส่วนบุคคล ความรู้กระจายอยู่ในสัมภาษณ์ wiki โฟลเดอร์คลาวด์ เอกสารทุน บันทึกนิทรรศการ สื่อเวิร์กช็อป โซเชียลมีเดีย และประสบการณ์เล่าปากต่อปาก", "Non-Governmental Matters เป็นวัสดุภาคสนามชั้นแรกเกี่ยวกับ Hackteria, ค่ายศิลปะ-เทคโนโลยีข้ามชาติ การศึกษาอิสระ โมเดลทุน การทดลองวัสดุ และความต่างทางวัฒนธรรม Peach Blossom Spring แปลงสิ่งเหล่านี้เป็นปัญหาการเก็บรักษาความรู้ในยุค AI", "ลูปปัจจุบันคือ สำรวจ Peach Blossom Spring คุยกับความทรงจำสัมภาษณ์ของ NPC ถาม campfire LLM Wiki แล้วสร้างซีนที่เปิดเส้นทางข้าม public source packet และ promoted wiki memory ส่วน question lint มาจาก shared memory traversal และบอกความเฉพาะ ความพร้อมของหลักฐาน และศักยภาพข้ามระบบ"],
...
const CAMPFIRE_DIALOGUE_COPY: Record<LanguageCode, { name: string; kicker: string; subtitle: string; playerSpeaker: string; intro: string; fail: string; failError: string; needQuestion: string; sourceTitle: string; sourceLinks: string; noLinks: string; suggestions: string; placeholder: string; suggest: string; zine: string; thinking: string }> = {
  "zh-TW": {
    ...
    placeholder: "問：想找藝術/生物倫理、DIY synth、獨立藝術營或替代教育案例嗎？",
    suggest: "建議一個問題",
    zine: "維基小書",
  ...
  en: {
    ...
    placeholder: "Ask about art/bioethics, DIY synths, independent art camps, or alternative education…",
  ...
  id: {
    ...
    placeholder: "Tanya tentang seni/bioetika, synth DIY, camp seni independen, atau pendidikan alternatif…",
  ...
  de: {
    ...
    placeholder: "Frage nach Kunst/Bioethik, DIY-Synths, unabhängigen Kunst-Camps oder alternativer Bildung…",
  ...
  ja: {
    ...
    placeholder: "芸術/生命倫理、DIY シンセ、独立アートキャンプ、代替教育について聞く…",
    suggest: "質問を提案",
    zine: "Wiki zine",
  ...
  th: {
    ...
    placeholder: "ถามเรื่องศิลปะ/bioethics, DIY synth, independent art camp หรือ alternative education…",
    suggest: "เสนอคำถาม",
    zine: "ซีน wiki",
