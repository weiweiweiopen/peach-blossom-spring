...
export async function askDeepSeekPbsQuestionSuggestions({ preferredLanguage, seedQuestions }: AskPbsQuestionSuggestionsArgs): Promise<string[]> {
  const systemPrompt = trimMessage([
    ...
    'The audience may know nothing about NGM, Hackteria, SGMK, DIY biology, DIY synths, independent art camps, or alternative education.',
    ...
    'Good question patterns include: finding cases that connect artistic expression and bioethics; why hacker camps often include DIY synths; what an independent art camp is; why camps can work as alternative education.',
...
export async function askDeepSeekPbsCampfire({ question, preferredLanguage, sharedMemoryContext }: AskPbsCampfireArgs): Promise<string> {
  const systemPrompt = trimMessage([
    ...
    'Keep the full answer within 230 Chinese characters when replying in Traditional Chinese, or within about 120 English words otherwise, because the PBS tip is required.',
    '',
