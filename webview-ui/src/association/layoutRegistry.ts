export type AssociationLayoutId = 'editorial_spread' | 'matrix_typographic_wall' | 'pbs_reset_title_kinetic' | 'soft_sound_commons' | 'aino_motion_grid' | 'aino_tui_blocks' | 'glyph_mask_flow';

export interface AssociationLayoutPreset {
  id: AssociationLayoutId;
  name: string;
  intent: string;
  minColumns: number;
  maxColumns: number;
...
export const ASSOCIATION_LAYOUT_PRESETS: AssociationLayoutPreset[] = [
  {
    id: 'editorial_spread',
...
export function getAssociationLayoutPreset(id: AssociationLayoutId): AssociationLayoutPreset {
  const preset = ASSOCIATION_LAYOUT_PRESETS.find((layout) => layout.id === id);
