...
import type { AssociationHtmlLayoutVariant } from "./publicArtifactHtml.js";
...
function allowedUiCorpus(): AssociationCorpus {
  const cards = [...entryNoteCards(), ...associationCorpus.cards.filter(isAllowedZineCard)];
  const ids = new Set(cards.map((card) => card.id));
  return {
    cards,
    edges: associationCorpus.edges.filter((edge) => ids.has(edge.source) && ids.has(edge.target)),
    manifest: associationCorpus.manifest,
  };
}
...
async function loadCompiledWikiIndex(): Promise<CompiledWikiNote[]> {
  // Old compiled PBS wiki / NotebookLM-derived RAG is no longer a zine input.
  // The association writer receives only the source-first packet produced from
  // pbs-local-memory / pbs_engine exports. Keep this async boundary so older
  // callers do not need to change shape.
  if (!compiledWikiIndexPromise) compiledWikiIndexPromise = Promise.resolve([]);
...
function buildEditorialMessages(query: string, workflow: Workflow, language: AssociationZineLanguage, compiledNotes: CompiledWikiNote[] = []) {
  ...
  const user = JSON.stringify({
    ...
    editorialMode: "source-first association zine rewrite",
    desiredAngles: [
      "從玩家提供的問題出發，不要套用固定題材、預設領域或上一份小誌的成功形式。",
      "全文只服務同一個中心問題：先判斷材料能形成論點，還是只能形成閱讀路徑；不要把後者偽裝成前者。",
      "把 pbs_engine.py 已找到的答案與 evidence 改寫成更有創意和洞見的語言；支持、限制、缺口與未來查證方向都要清楚，不要填充漂亮句子。",
      "只使用 sourceObservations、deepReadObservations 與 linkedEvidenceTrails 裡真的出現的頁名、詞彙、材料與方法。",
      "不要使用舊 wiki memory 或 NotebookLM 語境；文章必須只改寫當次 source-first packet 裡的實際作品、工作坊、方法、材料或社群實踐。",
      "如果頁面最有價值的是作品/方法清單，就直接整理成可玩的閱讀路徑；不要硬寫成宏大宣言。",
      "小誌不是檢索報告，也不是完成論文；每一章都要有作者判斷、場景、矛盾、缺口或下一步。",
      "不要把頁面格式誤讀成頁面意義：有時間表、材料清單、手冊章節，不等於它的核心是教學流程；駐村頁、資助頁、交換頁、人物經歷頁，必須先判斷它的社會功能，再判斷技術內容。",
      "如果頁面是 micro-residency / residency / artist-in-residence / fellow / exchange / mobility，優先理解為獨立藝術家取得實驗室、機構、技術單位、跨國網絡或資源通道的案例；不要自動說成 collective exploration、工作坊精神或教學方法。",
      "除非頁面明確寫出 collective、collaborative workshop、共同製作或共同研究，不要把個人駐村、個人研究紀錄、機構合作通道寫成集體探索。",
...
function cleanLLMText(value: unknown): string {
  return String(value ?? "")
    ...
    .replace(/PBS\s+Association\s+corpus/gi, "Peach Blossom Spring shared memory")
    .replace(/Association\s+corpus/gi, "shared memory collection")
    .replace(/\bAssociation\b/gi, "shared memory")
...
function normalizeLLMArtifact(data: any): AssociationPublicArtifactContent {
...
function isLocalBrowser(): boolean {
  const windowRef = (globalThis as { window?: { location?: { hostname?: string } } }).window;
...
function configuredProxyUrl(): string {
  ...
  const configured = documentRef?.querySelector
    ? documentRef.querySelector('meta[name="pbs-chat-api"], meta[name="sow-chat-api"]')?.getAttribute?.("content")?.trim()
    : "";
  if (isLocalBrowser()) return "/api/chat/association";
  return configured || DEFAULT_DEEPSEEK_PROXY_URL;
}

function usesLocalAssociationProxy(url: string): boolean {
  return url === "/api/chat/association";
}
...
async function requestDeepSeekJson(system: string, user: string, maxTokens = 900, temperature = 0.9): Promise<any> {
  ...
  try {
    ...
    const isLocalProxy = usesLocalAssociationProxy(proxyUrl);
    const localApiKey = isLocalProxy ? getInitialDeepSeekApiKey() : "";
    response = await fetch(proxyUrl, {
      ...
      body: JSON.stringify(isLocalProxy
        ? {
          systemPrompt: `${system}
...
async function callDeepSeekEditorialWriter(query: string, workflow: Workflow, language: AssociationZineLanguage, compiledNotes: CompiledWikiNote[], onProgress?: AssociationProgressCallback, options: BrowserAssociationOptions = {}): Promise<AssociationPublicArtifactContent> {
  ...
  const sections: AssociationPublicArtifactContent["sections"] = [];
  for (let index = 0; index < ZINE_SECTION_COUNT; index += 1) {
    ...
    const requestSection = (rewrite = false): Promise<any> => requestDeepSeekJsonWithRetry(
        `${languageInstruction(language)}\n${articleLength}\n只生成第 ${index + 1}/${ZINE_SECTION_COUNT} 章 JSON：{"id":"","title":"","body":"","pullQuote":""}。這一章必須完成 sectionFocus.sectionJob，優先使用 sectionFocus.primaryPages 與 sectionFocus.relationTrail，不要平均重複其他章。必須至少使用一個實際頁名、作品名、事件、概念、社群實踐或方法，並明確說它能如何幫助或限制 query；若材料不足，這章要寫成閱讀路徑、缺口或查證問題，不要幻想新事實或堆抽象詞。除非 wantsMakingTutorial=true，不要寫成工具製作、教學步驟、BOM 或工作坊流程。這是 PBS wiki 導讀段落，不是檢索報告也不是硬湊的論文：要寫出判斷、場景、矛盾、缺口或下一步，而不是列出索引。後半段必須延續閱讀路徑或論證，處理反證、限制、比較或未來研究方向，不要突然轉成材料清單、造句式結尾或小誌生成方法說明。不要寫系統/流程語，不要寫任何人名。不要輸出 Association、corpus、workflow、debug、prompt、source trail、SourceNotes、NotebookLM bridge、primarySourcePacket、compiled notes；面向讀者時改稱共享記憶、主題筆記、實體筆記、閱讀路徑。${rewrite ? "上一版和前文太像，請換用不同頁名、不同用途、不同句型重寫；不要保留相同開頭或相同結論。" : ""}`,
      JSON.stringify({
        query,
        title,
        subtitle,
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    try {
      item = await requestDeepSeekJsonWithRetry(
        `${languageInstruction(language)}\n${articleLength}\n只生成第 ${index + 1} 個 protocol JSON，不要陣列：{"title":"","body":""}。預設寫成一個具體研討會下一步：證據檢查、反例搜尋、比較問題、未來研究問題或點開頁面後能確認的事；只有 wantsMakingTutorial=true 才能寫製作/實作步驟。不要寫系統/流程語，不要寫任何人名。不要輸出 Association、corpus、workflow、debug、prompt、source trail；面向讀者時改稱共享記憶、主題筆記、實體筆記、閱讀路徑。`,
...
function articleCharacterCount(artifact: AssociationPublicArtifactContent): number {
...
function createBrowserWorkflow(query: string): Workflow {
  ...
  try {
    ...
    return runAssociationWorkflow(expandedQuery, corpus);
  } catch (error) {
    ...
    try {
      ...
      return runAssociationWorkflow(query, corpus);
    }
  }
}
...
export async function generateBrowserAssociationZine(query: string, language: AssociationZineLanguage = "zh-TW", onProgress?: AssociationProgressCallback, options: BrowserAssociationOptions = {}): Promise<BrowserAssociationResult> {
  ...
  const variant: AssociationHtmlLayoutVariant = "pbs-reset-title";
  let artifact: AssociationPublicArtifactContent;
  try {
    ...
    console.error("Association editorial writer failed; not rendering a short template fallback.", error);
    persistClickTrace(buildClickTrace({ requestId, query, language, workflow, errorClass: errorClass(error, "association_writer_failed"), errorMessage: errorMessage(error) }));
    throw error;
  }
