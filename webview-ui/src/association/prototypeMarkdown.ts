import type { AssociationCorpus, AssociationReport, SourceCard } from "./engine.js";
import type { SemanticVectorContext } from "./connectedPapers.js";
import { buildSemanticVectorContext } from "./connectedPapers.js";
import { generateAssociationReport } from "./engine.js";
...
export function generatePrototypeWikiPage(seed: string, corpus: AssociationCorpus): PrototypeWikiPage {
  const report = generateAssociationReport(seed, corpus);
...
function renderEditorialRelationPathsPrompt(report: AssociationReport): string {
...
function renderDeepReadPrompt(report: AssociationReport): string {
...
function renderDeepReadMarkdown(report: AssociationReport): string {
  if (report.deepReadCards.length === 0) {
...
function renderLinkedEvidenceMarkdown(report: AssociationReport): string {
  if (report.linkedCards.length === 0) {
...
function renderDepthGateMarkdown(report: AssociationReport): string {
...
function perceptionTargetsForCard(card: SourceCard, corpus: AssociationCorpus): string[] {
...
function buildProjectTitle(report: AssociationReport): string {
...
function translateConfidence(confidence: AssociationReport["futures"][number]["confidence"]): string {
...
function slugify(value: string): string {
  return value
    ...
    .replace(/^-+|-+$/g, "") || "association-prototype-page";
}
