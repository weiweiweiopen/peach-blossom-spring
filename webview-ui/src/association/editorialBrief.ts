...
import type { AssociationOutputPlan } from "./associationWorkflow.js";

export interface AssociationEditorialBriefSection {
  id: string;
  title: string;
  body: string;
...
export interface AssociationEditorialBrief {
  schemaVersion: "association-editorial-brief-v1";
  seed: string;
  title: string;
  deck: string;
  selectedTopic?: Pick<
  ...
  >;
  sections: AssociationEditorialBriefSection[];
...
export function buildEditorialBrief(params: {
  ...
  const brief: AssociationEditorialBrief = {
    schemaVersion: "association-editorial-brief-v1",
    seed,
...
export function validateEditorialBrief(brief: AssociationEditorialBrief): void {
  const visible = [
    brief.title,
    brief.deck,
...
function buildReadableSections(
  ...
  outputPlan: AssociationOutputPlan,
  evidenceCards: SourceCard[],
): AssociationEditorialBriefSection[] {
