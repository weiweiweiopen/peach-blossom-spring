...
const modeLabels: Record<FinalDocumentMode, string> = {
  ...
  manufacturing_technical_file: 'Wiki Technical File',
  travel_plan: 'Wiki Travel Plan',
  philosophical_debate: 'Wiki Debate',
};
...
function bodyFromEvidence(question: string, evidence: ChatEvidence[], mode: FinalDocumentMode): string {
  ...
  return [
    `原始問題：「${compact(question, 120)}」。這份報告只使用閱讀史、訪談、wiki、網站資料與公開材料進行整理，不呼叫外部 API。`,
    `核心線索：${conceptLine}。`,
    sourceFrame,
...
export function generateWikiDaydreamReport(args: WikiDaydreamReportArgs): FinalDocument {
  ...
  const question = args.question || args.pet?.question.text || 'Untitled note';
