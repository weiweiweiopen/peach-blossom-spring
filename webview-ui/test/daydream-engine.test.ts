...
import { generateDaydreamReport, parseSeedKeywords } from '../src/daydream/engine.js';
import { generatePrototypeWikiPage } from '../src/daydream/prototypeMarkdown.js';

test('daydream parser extracts seed keywords', () => {
...
test('daydream parser maps Chinese seed terms to corpus keywords', () => {
...
test('daydream report retrieves source cards and generates cited futures', () => {
  const report = generateDaydreamReport(
...
test('daydream report marks low evidence seeds as low confidence', () => {
  const report = generateDaydreamReport('zzzzqqqq xxyyqqq nnnnomatch', daydreamCorpus);
...
test('daydream prototype renders a markdown wiki page with mapping explanation', () => {
  ...
  assert.match(page.llmPrompt, /# Daydream LLM 生成提示/);
  assert.match(page.llmPrompt, /## 收集到的 Wiki／來源頁/);
  assert.match(page.llmPrompt, /## Recursive Link Reading \/ 深讀鏈/);
  assert.match(page.llmPrompt, /## Depth Gate \/ 深度門檻/);
  assert.match(page.llmPrompt, /## Connected Papers 語義向量層/);
  assert.match(page.llmPrompt, /## 湧現規則/);
  assert.match(page.llmPrompt, /不要只是摘要來源頁面/);
