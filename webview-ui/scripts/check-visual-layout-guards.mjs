...
const checks = [
  ...
  ["Dialogue avatars share one compact sprite slot", /rpg-dialogue-avatar-sprite/.test(app) && /rpg-dialogue-avatar-sprite/.test(rpgDialogue) && /rpg-dialogue-avatar-sprite[\s\S]*width:\s*54px[\s\S]*height:\s*54px[\s\S]*overflow:\s*visible/.test(uiSystemContract) && !/rpg-dialogue-avatar-frame/.test(app + rpgDialogue + uiSystemContract) && !/rpg-dialogue-avatar-frame[\s\S]*(?:72|92|174|184)px/.test(css + uiSystemContract)],
  ...
  ["Dialogue suggestions show playable questions", /COMMUNITY_QUERY_PROMPTS:\s*Record<LanguageCode, string\[]>/.test(app) && /Hackteria, SGMK, or KOBAKANT/.test(app) && /sourceBridgeQuestions[\s\S]*我在 PBS 裡想做一個小型獨立社群/.test(rpgDialogue) && /englishSourceBridge[\s\S]*small independent community in PBS/.test(rpgDialogue)],
  ["NPC suggestions avoid transcript sentence prompts", !/function transcriptQuestionSeed/.test(rpgDialogue) && !/responseQuestions/.test(rpgDialogue) && !/訪談記憶/.test(rpgDialogue) && /我在 PBS 裡想做一個小型獨立社群/.test(rpgDialogue)],
  ["NPC suggestions are player-facing game questions", /我想做一個照護農地與蜂群的小工具/.test(suggestedQuestionBlock) && /公開案例/.test(suggestedQuestionBlock) && /我只有一個模糊靈感/.test(suggestedQuestionBlock) && !/訪談|工作坊經驗|玩家想|入口|改寫|\bsource\b|\bsources\b/i.test(npcVisiblePromptCopy)],
  ...
  ["Campfire uses shared compact sprite slot", !/rpg-dialogue-avatar-frame|rpg-dialogue-avatar-frame--campfire/.test(app + rpgDialogue + css + uiSystem) && !/width:\s*184px/.test(uiSystem + css) && /rpg-dialogue-avatar-sprite[\s\S]*overflow:\s*visible/.test(uiSystem) && /<div className="rpg-dialogue-avatar-sprite" aria-label=\{label\}>[\s\S]*<img[\s\S]*className="block h-auto max-h-full w-auto object-contain object-center"/.test(app)],
  ...
  ["Player dialogue avatar matches NPC pixel sprite", /function DialoguePixelAvatar[\s\S]*className="rpg-dialogue-avatar-sprite"[\s\S]*className="rpg-dialogue-avatar-pixels"[\s\S]*gridTemplateColumns:[\s\S]*2px[\s\S]*gridAutoRows:\s*"2px"[\s\S]*style=\{\{ backgroundColor: color \|\| "transparent" \}\}/.test(app) && /function PixelAvatar[\s\S]*className="rpg-dialogue-avatar-sprite"[\s\S]*className="rpg-dialogue-avatar-pixels"[\s\S]*gridTemplateColumns:[\s\S]*2px[\s\S]*gridAutoRows:\s*'2px'[\s\S]*style=\{\{ backgroundColor: color \|\| 'transparent' \}\}/.test(rpgDialogue)],
