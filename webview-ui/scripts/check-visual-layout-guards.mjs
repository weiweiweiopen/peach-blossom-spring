...
const checks = [
  ...
  ["Dialogue suggestions show player-to-person questions", /COMMUNITY_QUERY_PROMPTS:\s*Record<LanguageCode, string\[]>/.test(app) && /Hackteria, SGMK, or KOBAKANT/.test(app) && /sourceBridgeQuestions[\s\S]*你剛剛談到的核心概念/.test(rpgDialogue) && /englishSourceBridge[\s\S]*Could you explain the core idea/.test(rpgDialogue)],
  ["NPC suggestions avoid transcript sentence prompts", !/function transcriptQuestionSeed/.test(rpgDialogue) && !/responseQuestions/.test(rpgDialogue) && !/訪談記憶/.test(rpgDialogue) && /你覺得我下一個最值得追問的問題是什麼/.test(rpgDialogue)],
  ["NPC suggestions are transcript-guided player questions", /我想知道農業機器人可以用來幹嘛/.test(suggestedQuestionBlock) && /什麼是電子織品/.test(suggestedQuestionBlock) && /你剛剛談到的核心概念/.test(suggestedQuestionBlock) && !/工作坊經驗|玩家想|入口|改寫|\bsource\b|\bsources\b|這個 NPC/i.test(npcVisiblePromptCopy)],
  ["PBS prompts are player-helpful experiences", /理解/.test(npcVisiblePromptSurface) && /社群的資源|公開材料|社群資源/.test(npcVisiblePromptSurface) && /靈感|啟發|成熟|可查證/.test(npcVisiblePromptSurface) && /社群/.test(npcVisiblePromptSurface)],
