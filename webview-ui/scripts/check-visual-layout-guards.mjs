...
const checks = [
  ...
  ["Dialogue suggestions show newcomer player-to-person questions", /COMMUNITY_QUERY_PROMPTS:\s*Record<LanguageCode, string\[]>/.test(app) && /Hackteria, SGMK, or KOBAKANT/.test(app) && /sourceBridgeQuestions[\s\S]*你是誰？你主要在做什麼/.test(rpgDialogue) && /englishSourceBridge[\s\S]*Who are you, and what do you mainly work on/.test(rpgDialogue)],
  ...
  ["NPC suggestions are beginner-guided player questions", /我想知道農業機器人可以用來幹嘛/.test(suggestedQuestionBlock) && /什麼是電子織品/.test(suggestedQuestionBlock) && /KOBAKANT 是什麼/.test(suggestedQuestionBlock) && /Hackteria 是什麼/.test(suggestedQuestionBlock) && !/你提到|你剛剛|這段談話|共同工作方式|工作坊經驗|玩家想|入口|改寫|\bsource\b|\bsources\b|這個 NPC/i.test(npcVisiblePromptCopy)],
  ["PBS prompts are player-helpful newcomer experiences", /基本的問題|完全不懂|是什麼/.test(npcVisiblePromptSurface) && /材料|工具|社群/.test(npcVisiblePromptSurface) && /例子|作品|活動/.test(npcVisiblePromptSurface) && /社群/.test(npcVisiblePromptSurface)],
  ...
  ["NPC follow-ups keep evidence anchors", /function evidenceAnchor/.test(rpgDialogue) && /function evidenceKeyword/.test(rpgDialogue) && /你可以用「\$\{keyword\}」再多舉一個具體作品或材料例子嗎/.test(rpgDialogue) && /如果我接著讀「\$\{anchor\}」/.test(rpgDialogue) && !/這題可以怎麼變成我真的能嘗試的一個小任務/.test(rpgDialogue)],
