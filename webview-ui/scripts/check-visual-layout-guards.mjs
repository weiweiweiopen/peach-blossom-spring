...
const checks = [
  ...
  ["Dialogue suggestions show playable questions", /COMMUNITY_QUERY_PROMPTS:\s*Record<LanguageCode, string\[]>/.test(app) && /Hackteria, SGMK, or KOBAKANT/.test(app) && /sourceBridgeQuestions[\s\S]*第一次認識這個人/.test(rpgDialogue) && /englishSourceBridge[\s\S]*new to this person/.test(rpgDialogue)],
  ["NPC suggestions avoid transcript sentence prompts", !/function transcriptQuestionSeed/.test(rpgDialogue) && !/responseQuestions/.test(rpgDialogue) && !/訪談記憶/.test(rpgDialogue) && /第一次認識這個人/.test(rpgDialogue)],
  ["NPC suggestions are transcript-understanding questions", /我想知道農業機器人可以用來幹嘛/.test(suggestedQuestionBlock) && /什麼是電子織品/.test(suggestedQuestionBlock) && /第一次認識這個人/.test(suggestedQuestionBlock) && !/工作坊經驗|玩家想|入口|改寫|\bsource\b|\bsources\b/i.test(npcVisiblePromptCopy)],
  ...
  ["NPC visible prompts avoid system language", !/人格|source index|syntax|markdown|路線|工作方法自述|入口|改寫|\bsource\b|\bsources\b|玩家想|更成熟、更可查證、也更好玩/i.test(npcVisiblePromptCopy)],
