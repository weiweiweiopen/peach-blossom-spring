...
const checks = [
  ...
  ["Zine template labels are multilingual", /function templateCopy[\s\S]*Klangkarte[\s\S]*音の地図[\s\S]*แผนที่เสียง/.test(template) && /copy\.map/.test(template) && /copy\.sequence/.test(template) && /copy\.closing/.test(template)],
  ...
  ["Zine print prefers under target over 17-page overflow", /let bestUnder = null/.test(generator) && /if \(best\.pages > targetPages && bestUnder\) best = bestUnder/.test(generator)],
  ["Zine print hides feedback before calibration", /beforeprint[\s\S]*setPrintMode\(true\); calibrate/.test(generator) && /\.zine-feedback-page\[hidden\][\s\S]*height:\s*0/.test(generator)],
  ...
  ["Dialogue suggestions show three safer questions", /return shuffled\.slice\(0, 3\)/.test(app) && !/avoid becoming publicity|變成宣傳|reine Werbung|宣伝化/.test(app) && /return \[attemptsQuestion, fallback\[language\], method\[language\]\]/.test(rpgDialogue) && /material practices/.test(rpgDialogue)],
  ...
  ["Campfire header copy stays canonical", /LLM WIKI CAMPFIRE/.test(app) && /多重心智自我火燄/.test(app) && /Association \/ 聯想 shared-fire terminal/.test(app) && !/The Multi-Minds Self Campfire<\/h/.test(app)],
