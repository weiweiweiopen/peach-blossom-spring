...
const checks = [
  ...
  ["Zine generation avoids page-count padding", /articleLengthInstruction[\s\S]*no required page multiple[\s\S]*Do not pad text for printing/.test(generator) && !/ZINE_PRINT_PAGE_MULTIPLE|ZINE_TARGET_PRINT_PAGES|PRINT BINDING TARGET/.test(generator)],
  ...
  ["Zine section prose is compact", /Each section body should be concise/.test(generator) && /visible text thin:/.test(generator) && !/body 必須符合/.test(generator)],
