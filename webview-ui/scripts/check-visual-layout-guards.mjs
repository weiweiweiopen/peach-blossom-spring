...
const dialogueAvatar = readFileSync(join(root, "src", "components", "DialogueAvatarSprite.tsx"), "utf8");
...
const npcVisiblePromptCopy = Array.from(npcVisiblePromptSurface.matchAll(/`([^`]+)`|'([^']+)'|"([^"]+)"/g))
...
const checks = [
  ...
  ["Campfire wiki intro is localized", /CAMPFIRE_DIALOGUE_COPY[\s\S]*ja:\s*{[\s\S]*火が共有記憶/.test(app)],
  ...
  ["Japanese campfire/NPC body is capped", /data-language="ja"\] \.rpg-dialogue-panel \.rpg-dialogue-message[\s\S]*font-size:\s*clamp\(18px,\s*1\.25vw,\s*24px\)/.test(css)],
  ...
  ["Campfire submit keeps emoji while busy", /aria-busy=\{isThinking\}[\s\S]*>💬<\/button>/.test(app)],
  ...
  ["Campfire copy covers all languages", /CAMPFIRE_DIALOGUE_COPY[\s\S]*"zh-TW":[\s\S]*en:[\s\S]*id:[\s\S]*de:[\s\S]*ja:[\s\S]*th:/.test(app)],
  ...
  ["Campfire uses zine footer recipe", /className="rpg-dialogue-form flex gap-4" data-ui-footer="zine"/.test(app)],
  ["Campfire controls are contract-marked", /data-ui-control="icon-button"[\s\S]*aria-label=\{copy\.suggest\}[\s\S]*data-ui-control="icon-button"[\s\S]*>💬<\/button>[\s\S]*data-ui-control="icon-button"[\s\S]*>📚<\/button>/.test(app)],
  ...
  ["PBS 2026.2 uses Cloud D1 source memory online", /export-d1-sql/.test(pbsEngine) && /pbs-memory-worker/.test(projectReadme + localMemoryGame) && /Cloudflare D1 SQLite \/ FTS source index/.test(projectReadme + localMemoryGame) && /\/api\/memory\/draft[\s\S]*stored`?\s*(?:is|:)\s*`?false/.test(localMemoryGame)],
  ["PBS 2026.2 keeps local full-memory source-first", /export-game-index/.test(pbsEngine) && /Runtime search is source-first/.test(pbsEngine) && /run_pbs_local_game\.sh/.test(projectReadme + localMemoryGame) && /Sources\/Raw canonical sources/.test(pbsEngine)],
  ...
  ["Dialogue avatars share one compact sprite slot", /rpg-dialogue-avatar-sprite/.test(dialogueAvatar) && /CharacterDialogueAvatar/.test(app + rpgDialogue) && /ImageDialogueAvatar/.test(app) && /rpg-dialogue-avatar-sprite[\s\S]*width:\s*54px[\s\S]*height:\s*54px[\s\S]*overflow:\s*visible/.test(uiSystemContract) && !/rpg-dialogue-avatar-frame/.test(app + rpgDialogue + dialogueAvatar + uiSystemContract) && !/rpg-dialogue-avatar-frame[\s\S]*(?:72|92|174|184)px/.test(css + uiSystemContract)],
  ...
  ["Campfire is animated collidable 4x4 wiki entry", /MULTI_MIND_CAMPFIRE_6/.test(campfireManifest) && /"footprintW": 4/.test(campfireManifest) && /"footprintH": 4/.test(campfireManifest) && /CAMPFIRE_DIALOGUE_COPY[\s\S]*name:\s*"多重心智自我火燄"/.test(app) && /interactiveFurnitureTypes/.test(officeCanvas) && /getAnimationFrames\(item\.type\)/.test(officeState)],
  ["Campfire interaction uses bottom stone row", /function campfireStoneBoundsFromLayout[\s\S]*row: bounds\.row \+ bounds\.h - 1[\s\S]*isCampfireTile/.test(app)],
  ...
  ["Dialogue suggestions show wiki-grounded questions", /COMMUNITY_QUERY_PROMPTS:\s*Record<LanguageCode, string\[]>/.test(app) && /BioElectronics and BioMaterials Workshop, ISI Yogyakarta 2019/.test(app) && /sourceBridgeQuestions[\s\S]*電子織品、柔性電路或穿戴電子/.test(rpgDialogue) && /bridgeByLanguage[\s\S]*e-textile, soft-circuit, or wearable-electronics/.test(rpgDialogue) && /bridgeByLanguage[\s\S]*CoLabs Chiang Mai คืออะไร/.test(rpgDialogue)],
  ["NPC suggestions avoid transcript sentence prompts", !/function transcriptQuestionSeed/.test(rpgDialogue) && !/responseQuestions/.test(rpgDialogue) && !/訪談記憶/.test(rpgDialogue) && !/我可以問你哪個最基本的問題/.test(rpgDialogue)],
  ["NPC suggestions are wiki-guided player questions", /Green Fab Lab Valldaura/.test(suggestedQuestionBlock) && /開放硬體、生態科技或無人機工作坊/.test(suggestedQuestionBlock) && /FarmBot/.test(suggestedQuestionBlock) && /什麼是電子織品/.test(suggestedQuestionBlock) && /柔性電路和電子合成器/.test(suggestedQuestionBlock) && /什麼是多點觸控介面/.test(suggestedQuestionBlock) && /Hackteria 是什麼/.test(suggestedQuestionBlock) && /Archive HOME MADE 和 Radionica krassfade/.test(suggestedQuestionBlock) && /Fluffy MIDI 和 Woolly Noise/.test(suggestedQuestionBlock) && /開源藝術社群/.test(suggestedQuestionBlock) && !/precision farming|Hackteria、SGMK 和 KOBAKANT 各自有哪些適合新手先讀|你提到|你剛剛|這段談話|共同工作方式|工作坊經驗|玩家想|入口|改寫|\bsource\b|\bsources\b|這個 NPC|你是誰|基本的問題/i.test(npcVisiblePromptCopy)],
  ["PBS prompts are player-helpful wiki experiences", /材料|工具|社群/.test(npcVisiblePromptSurface) && /例子|作品|活動|案例/.test(npcVisiblePromptSurface) && /電子織品|柔性電路|多點觸控|DIY synth|Hackteria|KOBAKANT/.test(npcVisiblePromptSurface)],
  ["NPC visible prompts avoid system language", !/NPC|人格|source index|syntax|markdown|路線|工作方法自述|入口|改寫|\bsource\b|\bsources\b|玩家想|更成熟、更可查證、也更好玩/i.test(npcVisiblePromptCopy)],
  ["NPC follow-ups keep evidence anchors", /function evidenceAnchor/.test(rpgDialogue) && /function evidenceKeyword/.test(rpgDialogue) && /makeFollowUpQuestions\(language, trimmed, answer\.evidence, persona\)/.test(rpgDialogue) && /你可以用「\$\{keyword\}」再多舉一個具體作品或材料例子嗎/.test(rpgDialogue) && /如果我接著讀「\$\{anchor\}」/.test(rpgDialogue) && !/這題可以怎麼變成我真的能嘗試的一個小任務/.test(rpgDialogue)],
  ...
  ["Inline source links wrap safely on mobile", /\.rpg-dialogue-source-line\s*\{[\s\S]*overflow-wrap:\s*anywhere/.test(css) && /@media \(max-width:\s*700px\), \(pointer:\s*coarse\)[\s\S]*\.rpg-dialogue-source-line\s*\{[\s\S]*grid-template-columns:\s*auto minmax\(0, 1fr\)/.test(css) && /\.rpg-dialogue-source-body\s*\{[\s\S]*grid-column:\s*2/.test(css)],
  ["Campfire and NPC dialogue share compact header layout", /w-\[min\(1320px,84vw\)\] h-\[80vh\] min-w-\[min\(860px,calc\(100vw-24px\)\)\]/.test(app) && /w-\[min\(1320px,84vw\)\] h-\[80vh\] min-w-\[min\(860px,calc\(100vw-24px\)\)\]/.test(rpgDialogue) && /rpg-dialogue-heading-line min-w-0 flex flex-nowrap items-center gap-3 overflow-visible/.test(app) && /rpg-dialogue-heading-line min-w-0 flex flex-nowrap items-center gap-3 overflow-visible/.test(rpgDialogue) && !/rpg-dialogue-kicker-row|rpg-dialogue-heading-line[^"]*overflow-hidden/.test(app + rpgDialogue) && /rpg-dialogue-heading-line[\s\S]*overflow:\s*visible[\s\S]*padding:\s*4px 0 6px/.test(uiSystem) && /rpg-dialogue-question-drawer w-full border border-border bg-bg\/70 px-4 py-4/.test(app) && /rpg-dialogue-question-drawer w-full border border-border bg-bg\/70 px-4 py-4/.test(rpgDialogue)],
  ["Dialogue text uses one field-sized font", /rpg-dialogue-panel :where\([\s\S]*\[data-ui-part="title"\][\s\S]*\[data-ui-part="field"\][\s\S]*font-size:\s*var\(--ui-type-field\)[\s\S]*line-height:\s*var\(--ui-line-field\)/.test(uiSystem) && /rpg-dialogue-panel :where\(\[data-ui-control="text-button"\]\)[\s\S]*font-size:\s*var\(--ui-type-field\)/.test(uiSystem) && /rpg-dialogue-input\[data-ui-part="field"\][\s\S]*font-size:\s*var\(--ui-type-field\)/.test(uiSystem)],
  ["Campfire uses shared compact sprite slot", !/rpg-dialogue-avatar-frame|rpg-dialogue-avatar-frame--campfire/.test(app + rpgDialogue + dialogueAvatar + css + uiSystem) && !/width:\s*184px/.test(uiSystem + css) && /rpg-dialogue-avatar-sprite[\s\S]*overflow:\s*visible/.test(uiSystem) && /ImageDialogueAvatar[\s\S]*className="block h-auto max-h-full w-auto object-contain object-center"/.test(dialogueAvatar)],
  ["Dialogue avatar labels are hidden", !/max-w-\[110px\] truncate text-xs text-text-muted/.test(app + rpgDialogue) && !/rpg-dialogue-avatars span/.test(css)],
  ...
  ["Player dialogue avatar matches NPC pixel sprite", /function PixelSprite[\s\S]*className="rpg-dialogue-avatar-pixels"[\s\S]*gridTemplateColumns:[\s\S]*2px[\s\S]*gridAutoRows:\s*'2px'[\s\S]*style=\{\{ backgroundColor: color \|\| 'transparent' \}\}/.test(dialogueAvatar) && /CharacterDialogueAvatar/.test(app + rpgDialogue)],
  ...
  ["Boot loading title uses non-blocking pixel font size", /@font-face[\s\S]*font-display:\s*swap/.test(css) && /\.boot-loading-title[\s\S]*font-family:\s*var\(--font-pixel\) !important[\s\S]*font-size:\s*clamp\(26px,\s*4\.2vw,\s*42px\)/.test(css)],
  ["Campfire header copy is multilingual", /CAMPFIRE_DIALOGUE_COPY[\s\S]*"zh-TW":[\s\S]*name:\s*"多重心智自我火燄"[\s\S]*en:[\s\S]*name:\s*"The Multi-Minds Self Campfire"[\s\S]*id:[\s\S]*de:[\s\S]*ja:[\s\S]*th:/.test(app) && /<h2[\s\S]*>\{copy\.name\}<\/h2>/.test(app) && !/Association \/ 聯想 shared-fire terminal/.test(app)],
