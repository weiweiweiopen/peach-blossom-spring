...
const checks = [
  ...
  ["Dialogue suggestions show wiki-grounded questions", /COMMUNITY_QUERY_PROMPTS:\s*Record<LanguageCode, string\[]>/.test(app) && /Hackteria, SGMK, or KOBAKANT/.test(app) && /sourceBridgeQuestions[\s\S]*電子織品、柔性電路或穿戴電子/.test(rpgDialogue) && /englishSourceBridge[\s\S]*e-textile, soft-circuit, or wearable-electronics/.test(rpgDialogue)],
  ...
  ["NPC suggestions are wiki-guided player questions", /Green Fab Lab Valldaura/.test(suggestedQuestionBlock) && /precision farming/.test(suggestedQuestionBlock) && /FarmBot/.test(suggestedQuestionBlock) && /什麼是電子織品/.test(suggestedQuestionBlock) && /柔性電路和電子合成器/.test(suggestedQuestionBlock) && /什麼是多點觸控介面/.test(suggestedQuestionBlock) && /Hackteria 是什麼/.test(suggestedQuestionBlock) && !/你提到|你剛剛|這段談話|共同工作方式|工作坊經驗|玩家想|入口|改寫|\bsource\b|\bsources\b|這個 NPC|你是誰|基本的問題/i.test(npcVisiblePromptCopy)],
