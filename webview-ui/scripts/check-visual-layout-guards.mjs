...
const checks = [
  ...
  ["Zine template labels are multilingual", /function templateCopy[\s\S]*Klangkarte[\s\S]*音の地図[\s\S]*แผนที่เสียง/.test(template) && /copy\.map/.test(template) && /copy\.sequence/.test(template) && /copy\.nextStep/.test(template)],
  ...
  ["Zine renderer uses sixteen filled print pages", /sections = artifact\.sections\.slice\(0, 8\)/.test(template) && /const protocolPages = artifact\.protocol\.slice\(0, 4\)/.test(template) && /tracePage\(cards\.slice\(0, 5\), "retrieval-trace-a", true\)/.test(generator) && /tracePage\(cards\.slice\(5\), "retrieval-trace-b"\)/.test(generator)],
  ["Zine print forces one section per page", /\.page \{ break-after: page !important; page-break-after: always !important;[\s\S]*height: 257mm !important/.test(generator)],
