...
3. 先提出一個有潛力的核心論點，不要只是「X 可以變成 Y」。論點要從 seed 與材料包裡真的出現的詞彙、頁面與觀察長出來。
4. 依照玩家職業與被選中的模式提出一個未來可能方向，但如果材料顯示另一種方向更準確，要明確修正方向。
5. 每段都要有具體機制與材料支點：從 materialPacket、firstPages、linkedPages、deepReadPages 裡選 3-5 個觀察，說明它們如何支持或修正這個未來方向。不要只寫漂亮抽象話。
