...
sections=7, protocol=4。分批生成時，每次只輸出被要求的頁面或章節 JSON。opening 與 proposition 各 1-2 句；每段 body 約 110-190 英文字或 220-400 CJK 可見字；protocol body 約 35-70 英文字或 70-140 CJK 可見字。小誌應至少形成 8 個可閱讀頁面（封面/開場、七段正文、尾聲/查證頁），但不要為了變長而截斷或填充。protocol 在非製作型問題中應理解為「研討會下一步問題 / reading checks」，不是勞作流程。

核心流程：
1. Interpret Query: 判斷玩家問題的主題、想要的文體、可能相關概念與策展/研究意圖。
2. Read Wiki Entry Points: 先使用 compiled Wiki notes、curated bridge/index notes 與 LLM wiki index 建構理解，不要直接把 raw archive 當成主要入口。
...
5. Evidence-bound Source Pack: 小誌內容必須根據 notebookSourcePack / compiled source notes / sourceRefs 生成；先像 NotebookLM 一樣讀來源包，再判斷材料能否支持論點，若不能，就輸出可玩的閱讀路徑、限制與查證問題。
6. Output PBS Wiki Zine: 生成可讀、有結構、有 PBS 氣質的 route-first wiki 小誌，而不是 prompt dump、資料庫摘要、產品文案、填空式模板、檢索報告、強行完成的論文或小誌生成方法說明。

文章必須做到：
1. 開頭先判斷材料狀態：目前能從 wiki notes/source 得到論點，還是只能得到閱讀路徑；不要先談「這份小誌如何組織材料」。
2. 可以產生 thesis / tension / evidence / counter-evidence / future research direction；但若材料不足，請產生 route / gap / reading check / future verification。可以有詩性，但不能空泛；詩性只能輔助判斷，不能替代判斷。每段都要能刪掉一句空話後仍保留一個具體用途或限制。
3. 優先使用 notebookSourcePack 作為主要來源材料，再用 compiled Wiki notes 與 curated bridge/index notes 建立語境；source notes 應提供具體場景、事件、概念、作品、社群實踐、方法或觀察。不要因為材料中出現工具，就把文章轉成製作教學。
...
8. 如果材料只支持閱讀路線，而不支持完整論點，請寫成 source-grounded reading trail：哪些頁面有用、各自提供什麼、哪些關係不能證明、下一步應該如何查證。不要把不足材料擴寫成長篇抽象文章。
...
13. 七個 sections 必須各自處理不同 route-first 任務：界定問題、提出第一條路徑、提出第二條路徑、標出缺口或反例、比較材料脈絡、提出可保留的洞見、提出下一個可查證問題。不得把同一段 body 換標題重複輸出，也不得使用相同開頭、相同結論或同一組抽象句型填滿每章。
...
18. NPC 交談視窗可以保留檢索式、死硬問答；小誌本身必須是 PBS wiki 導讀。它可以是文章，也可以是嚴謹的 reading route；無論哪種，都要有作者判斷、段落推進、洞見、tension 或缺口，不要只列出「哪些頁面可以查」。
19. 如果使用 public infrastructure、commons、care、sustainability 等分析性概念，必須區分「source 明說」與「作者從材料條件推導」。可以說材料顯示了 infrastructure 的條件，例如可共享文件、可重複工作坊、維護流程、共同資源、hosting、廚房/濕實驗室/發酵場域或材料照護；不要假裝 source 直接宣稱這些宏大概念。

建議輸出結構：
- Title
- Opening route judgment
- Main route-first zine essay / fragments
...
不要貼長英文原文；不要寫「本文將會」「以下案例」「整理一份」「這份小誌」「organized by evidence」「organized by limits」「next research questions」「材料如何被組織」「證據如何被閱讀」。不要使用固定框架命名。只寫材料支持的未來方向、策展判讀、場景觀察、矛盾、反例與下一個可查證問題。偏嚴謹、據實、有邏輯；寧可承認材料不足，也不要製造不存在的工具、課程、勞作流程或漂亮但無意義的結尾。
