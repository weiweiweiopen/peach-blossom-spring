...
| Zine article frame | `webview-ui/src/association/officialTemplateRenderer.ts` | public zine HTML | prose width and article frame CSS live in `zineLayoutGovernanceCss()` |
| Zine system frame | `webview-ui/src/association/browserAssociationGenerator.ts`, `webview-ui/src/association/associationFeedback.ts` | reading path, feedback | `.zine-system-frame` separates system sections from prose |
